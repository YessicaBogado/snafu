...
RUN apk update && apk add cython-dev && apk add python3-dev && apk add g++ gcc && \
    apk add openjdk8 && apk add openjdk8-jre
RUN pip3 install pyinotify && apk add inotify-tools
...
CMD ["/bin/sh", "-c", "(/home/snafu/mon.sh &) && /home/snafu/snafu-control --deployer"]
